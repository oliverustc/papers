...
+ [原文链接](https://link.springer.com/chapter/10.1007/978-3-319-28166-7_1)
...
安全多方计算（Multiparty computation）可用于对多方私有输入的计算任务进行隐私友好的外包处理。计算任务被委托给若干计算参与方（computation parties）；只要被腐化的参与方不超过特定阈值（例如不超过半数），这些参与方就无法获知输入信息或产生错误输出。然而在许多场景中，上述保证仍不充分：我们需要即使所有计算参与方均被腐化时仍能确保正确性（correctness），并且要求即使未参与计算的其他方也能对正确性进行验证。满足这些附加特性的协议被称为"通用可验证（universally verifiable）"协议。

本文提出了一种新型的通用可验证多方计算安全模型，并基于门限同态加密系统（threshold homomorphic cryptosystem）给出了实用构造方案。同时，我们开发了一种用于联合生成非交互式零知识证明（non-interactive zero-knowledge proofs）的多方协议，该协议本身可能具有独立的研究价值。
